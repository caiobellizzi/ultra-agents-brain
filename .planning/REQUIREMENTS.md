...
- [ ] **DIAG-01**: Operator can read a written audit doc that traces every write path (memory.add, eval.record, knowledge.ingest, approval.create) from agent run → DB row → AgentOS API response
- [ ] **DIAG-02**: Operator can read a decision doc stating whether all 5 agents keep a shared `db_id` or move to per-agent isolation, with Agno-source-backed rationale

### MEM — Memory Surface

- [ ] **MEM-01**: After a chat-agent run with memory-worthy content, os.agno.com Memory tab shows ≥1 new entry within 5 seconds
- [ ] **MEM-02**: Memory entries are scoped correctly per the db_id decision (DIAG-02) — either per-agent or correctly attributed in shared workspace
- [ ] **MEM-03**: `enable_user_memories=True` (or equivalent Agno config) is verified active on agents that should accumulate memory
...
- [ ] **KNOW-01**: Vault `.md` content is indexed into the Agno knowledge table (pgvector) and visible via the AgentOS Knowledge tab
- [ ] **KNOW-02**: Agentic RAG hits during agent runs are recorded as knowledge-access events visible in the UI
- [ ] **KNOW-03**: Re-indexing vault content (vault reindex entry point) updates the knowledge table without duplicates
...
- [ ] **MON-01**: Daily-brief date-mismatch bug (monitor-filed items missed by brief) is fixed and covered by a regression test
- [ ] **MON-02**: Vault sync `--delete` flag no longer wipes VPS-generated inbox items (fix verified by test)
